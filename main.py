...
def _is_valid_groq_key(key: str | None) -> bool:
    if not key:
        return False
    return key not in {"your_groq_api_key_here", "gsk_xxxxxxxxxxxxxxxx"}
...
def health() -> dict[str, Any]:
    ...
    return {
        "ok": True,
        "groq_configured": _is_valid_groq_key(key),
...
async def upload_file(file: UploadFile = File(...)) -> JSONResponse:
    if rag is None:
        raise HTTPException(status_code=500, detail="GROQ_API_KEY missing; cannot run RAG.")
